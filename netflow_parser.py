...
def ranges_frequency_histogram(range_dict):
    netflow_tools.write_dict(range_dict, 'dates_range_frequency')
...
def remove_unrelevant_dates(mydict, start_date, end_date):
...
def pearson_correlation(mydict, minDates=10):
    ...
    for interface1, interface2 in interfacePairs:
        ...
        if len(identicalDates) >= minDates:
...
# pearson_dict = pearson_correlation(relevant_anomalies, 30)
# write_dict(pearson_dict, 'pearson')
netflow_cluster.cluster_interfaces('pearson', 0.3)
